...
# Present ONLY in Phase 2 (joined to a cluster) — sets disable-apiserver/
# disable-scheduler/disable-controller-manager. In Phase 1.5 standalone
# the witness is a single-node cluster and needs its own apiserver up,
# so this file is absent.
# shellcheck disable=SC2034
K3S_DISABLES_CONFIG_FILE="${K3S_CONFIG_DIR}/03-witness-disables.yaml"
