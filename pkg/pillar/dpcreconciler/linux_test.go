...
func TestAddKubeServiceRules(t *testing.T) {
	log.Println("Starting TestAddKubeServiceRules test")
	g := initTest(t)
	...
	log.Println("Adding initial service rules")
	initialRules := dpcReconciler.AddKubeServiceRules(initialServices)
	log.Printf("Initial rules created: %d", len(initialRules))
	...
	log.Println("Adding updated service rules")
	updatedRules := dpcReconciler.AddKubeServiceRules(updatedKubeUserServices)
	log.Printf("Updated rules created: %d", len(updatedRules))
	...
	log.Println("Checking rules for expected ports and IPs")

	// Dump all rules for debugging
	for i, rule := range updatedRules {
		ruleStr := fmt.Sprintf("%+v", rule)
		log.Printf("Rule %d: %s", i, ruleStr)

		// Debug the rule's match options
		log.Printf("Rule %d match options: %v", i, rule.MatchOpts)
		log.Printf("Rule %d description: %s", i, rule.Description)
	}
	...
	for _, rule := range updatedRules {
		...
		if strings.Contains(rule.Description, "Mark Kubernetes HTTP ingress traffic") {
			log.Println("Found HTTP ingress rule")
			...
			if strings.Contains(rule.Description, "192.168.1.2") {
				log.Println("Found HTTP ingress rule for specific IP 192.168.1.2")
	...
	log.Println("TestAddKubeServiceRules completed successfully")
}
...
func TestKubeACEService(t *testing.T) {
	log.Println("Starting TestKubeACEService test")
	g := initTest(t)
	...
	log.Println("Getting intended filter rules")
	...
	log.Printf("Found %d IPv4 rules", len(ipv4Rules))
	...
	for _, item := range ipv4Rules {
		...
		log.Printf("Found rule: %s (%s)", rule.RuleLabel, rule.Description)
		if rule.RuleLabel == "Allow K3s API Servier requests" {
			found = true
			apiServerRule = &rule
			log.Printf("Found K3s API Server rule: %+v", rule)
			break
		}
	}
	...
	log.Println("TestKubeACEService completed successfully")
}
